                ...
                if (reporte.fotoUrl != null)
                  Image.network(
                    ...
                    errorBuilder: (context, error, stackTrace) =>
                        Container(height: 160, color: Colors.grey.shade200, child: const Center(child: Icon(Icons.broken_image, size: 40, color: Colors.grey))),
                  )
                else
                   Container(height: 160, color: Colors.grey.shade200, child: Center(child: Icon(Icons.image_not_supported, size: 40, color: Colors.grey.shade400))),
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Chip(
                        avatar: Icon(
                          esPendiente ? Icons.hourglass_empty_outlined : Icons.check_circle_outline,
                          size: 16,
                          color: esPendiente ? Colors.orange.shade800 : Colors.green.shade800,
                        ),
                        label: Text(esPendiente ? 'Pendiente' : 'Verificado', style: const TextStyle(fontSize: 11)),
                        backgroundColor: esPendiente ? Colors.orange.shade100 : Colors.green.shade100,
                  ...
                  if (esPendiente) ...[
                    ...
                    SizedBox(
                      ...
                      child: reporte.usuarioActualUnido
                          ? ElevatedButton.icon( // Botón "UNIDO" (ahora permite quitar apoyo)
                              icon: isUnjoining // Muestra carga si se está quitando apoyo
                                  ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                                  : const Icon(Icons.check_circle, size: 18), // Icono de check
                              label: Text(
                              ...
                              onPressed: isLoadingAction ? null : onUnjoinTap, // Deshabilitar si cualquier acción carga
                              style: ElevatedButton.styleFrom(
                                backgroundColor: Colors.green.shade600, // Color verde
                                foregroundColor: Colors.white, // Texto blanco
                                padding: const EdgeInsets.symmetric(vertical: 8),
                                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), // Opcional: bordes menos redondeados
                              ),
                            )
                          : reporte.puedeUnirse
                              ? OutlinedButton.icon( // Botón "UNIRME"
                                  icon: isJoining // Muestra carga si se está uniendo
                                  ...
                                  label: Text(
                                    reporte.apoyosPendientes > 0
                                        ...
                                        : '¡Yo también! Unirme',
                                    style: TextStyle(
                                      color: theme.colorScheme.primary, // Color primario
                                      fontWeight: FontWeight.bold,
                                    ),
                                  ),
                                  onPressed: isLoadingAction ? null : onJoinTap, // Llama a onJoinTap
                                  style: OutlinedButton.styleFrom(
                                    side: BorderSide(color: theme.colorScheme.primary), // Borde primario
                                    padding: const EdgeInsets.symmetric(vertical: 8),
                                  ),
                                )
                              : ElevatedButton.icon( // Botón "ES TU REPORTE" (deshabilitado)
                                  icon: const Icon(Icons.person_outline, size: 18, color: Colors.white70),
                                  label: Text(
                                      reporte.apoyosPendientes > 0
                                  ...
                                  style: ElevatedButton.styleFrom(
                                      backgroundColor: Colors.blueGrey.shade400, // Color azul grisáceo
                                      padding: const EdgeInsets.symmetric(vertical: 8)),
                                ),
                    ),
                  ],
